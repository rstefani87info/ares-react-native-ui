...
export class Color {
  ...
  sumAlpha(a) {
    return Color.getFromRGBArray([this.r, this.g, this.b, this.a + a > 1 ? 1 : this.a + a]);
  }

  convertToCSSRGBAColor() {
...
export class Shade{
  ...
  constructor(color, step = 25.5) {
    this.step = step >= 0 ? step : step * -1;
    this.middle = color;
    this.light = color.getLighter(step);
    this.lighter = this.light.getLighter(step);
    this.lightest = this.lighter.getLighter(step);
    this.dark = color.getDarker(step);
    this.darker = this.dark.getDarker(step);
    this.darkest = this.darker.getDarker(step);
    this.nextToblack = this.darkest.getDarker(step);
    this.nextTowhite = this.lightest.getLighter(step);
    this.white = instances.white;
